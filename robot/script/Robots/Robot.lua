...
function Robot:reqSelectLack(lack)
    local data = {Lack = lack}
    self:request("SelectLack", json.encode(data), handler(self, self.onSelectLackRsp))
end
...
function Robot:onSelectLackRsp(cmd, str)
   print("onSelectLackRsp", cmd, str)
end

function Robot:onSelectLackNotify(cmd, str)
    local data = json.decode(str)
    dump(data, "onSelectLackNotify:")
    self.lackSelectCount = self.lackSelectCount or 0
    self.lackSelectCount = self.lackSelectCount + 1
    if self.lackSelectCount == 3 then
        if self.id == self.currPlayerID then
            self.handoutCardIdx = math.random(1,#self.cards.StandCards)
            self:reqPlayHand(self.cards.StandCards[self.handoutCardIdx])
        end
    end
end
...
function Robot:onPlayerSelectLack(cmd, str)
    local data = json.decode(str)
    dump(data, "onPlayerSelectLack:")
end
